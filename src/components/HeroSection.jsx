...
import { ImageOff, Github, Linkedin, ChevronDown, Compass, Globe, Server, BrainCircuit, Container } from 'lucide-react'
...
export function HeroSection() {
  ...
  // Capabilities Snapshot scroll-in observer
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && !capVisible) {
          setCapVisible(true);
          // Animate capability stats
    ...
    const handleMouseMove = (e) => {
      ...
      if (x >= 0 && x <= rect.width && y >= 0 && y <= rect.height) {
        cursorGlowRef.current.style.background =
          `radial-gradient(600px circle at ${x}px ${y}px, rgba(59, 130, 246, 0.15), transparent 40%)`;
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        {/* GRID: content + photo in same centered layout */}
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)] gap-12 items-start">
          {/* LEFT: Content */}
            ...
            <div className="animate-fade-up animation-delay-100">
              <h1 className="text-3xl md:text-4xl font-bold leading-tight mb-5 max-w-2xl">
                ...
                <span className="relative inline-block">
                  ...
                  <span className="absolute -bottom-1 left-0 w-full h-2 bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 rounded-lg -z-10" />
                </span>{' '}
                and full-stack platforms across real-world domains.
              </h1>
            </div>
          ...
          {/* RIGHT: Photo (now inside grid, same container) */}
                ...
                {domainEmojis.map((item, idx) =>
                  domainEmojis.slice(idx + 1).map((target, targetIdx) => (
                    <line
                      ...
                      y2="50%"
                      stroke={
                        activeEmoji === item.chipId || activeEmoji === target.chipId
                          ? 'rgba(59, 130, 246, 0.4)'
                          : 'rgba(59, 130, 246, 0.1)'
                      }
                      strokeWidth="1"
      ...
      {/* ═══════════ Capabilities Snapshot — Premium Redesign ═══════════ */}
      <div ref={capRef} className="container mx-auto px-4 mb-14 animate-fade-up animation-delay-400">
        <div className="relative bg-white/5 dark:bg-black/10 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-white/10 shadow-2xl shadow-primary/5 hover:shadow-primary/15 hover:border-primary/30 transition-all duration-700 overflow-hidden">
          ...
          {/* 3-column grid of pillars — individual glassmorphism cards */}
              ...
              <div className="relative flex flex-wrap gap-2">
                {buildingAcrossChips.map((chip, idx) => (
                  <span
                    ...
                    className={`px-3 py-1.5 rounded-full text-sm font-medium backdrop-blur-sm border transition-all duration-300 cursor-default ${capVisible ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'
                      } ${activeEmoji === chip.id || hoveredChip === chip.id
            ...
            <div className="relative group/card rounded-2xl p-5 md:p-6 bg-gradient-to-br from-purple-500/5 to-pink-500/5 border border-purple-500/10 hover:border-purple-500/30 transition-all duration-500 hover:shadow-lg hover:shadow-purple-500/10">
              ...
              <div className="relative flex flex-wrap gap-2">
                {whatIBuildChips.map((chip, idx) => (
                  <span
                    key={idx}
                    className={`px-2.5 py-1 rounded-full text-xs font-medium backdrop-blur-sm border transition-all duration-300 cursor-default ${capVisible ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'
          ...
          <div className="mt-8 pt-6 border-t border-white/10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {techStackGroups.map((group, gIdx) => (
                <div
                  key={gIdx}
                  className={`rounded-xl p-4 border border-white/10 hover:border-white/20 transition-all duration-500 ${capVisible ? 'animate-in fade-in slide-in-from-bottom-4' : 'opacity-0'
                    }`}
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {group.techs.map((tech, tIdx) => (
                      <span
                        key={tIdx}
                        className={`px-2.5 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${group.gradient.replace('from-', 'from-').replace('to-', 'to-')}/10 border border-white/10 hover:border-white/25 hover:scale-105 transition-all duration-300 whitespace-nowrap`}
      ...
      {/* Stats - Centered Row */}
      <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8 animate-fade-up animation-delay-600">
      ...
      {/* Buttons - Centered */}
      <div className="max-w-4xl mx-auto flex flex-wrap gap-4 mb-8 animate-fade-up animation-delay-800">
        <Button
          variant="default"
          className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 shadow-lg shadow-primary/20 group"
          asChild
        >
      ...
      {/* Social - Centered */}
      <div className="max-w-4xl mx-auto pb-6 animate-fade-up animation-delay-1000">
        <div className="flex items-center gap-4">
          <a
            href="https://github.com/ghantapavan93"
            target="_blank"
            rel="noopener noreferrer"
            className="h-10 w-10 flex items-center justify-center rounded-full text-white bg-[#24292e] hover:bg-[#24292e]/90 transition-all hover:scale-110"
            aria-label="GitHub"
          >
            <Github size={20} />
          </a>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="h-10 w-10 flex items-center justify-center rounded-full text-white bg-[#0077b5] hover:bg-[#0077b5]/90 transition-all hover:scale-110"
            aria-label="LinkedIn"
          >
            <Linkedin size={20} />
          </a>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="h-10 w-10 flex items-center justify-center rounded-full text-white bg-[#FF0000] hover:bg-[#FF0000]/90 transition-all hover:scale-110"
            aria-label="YouTube"
          >
